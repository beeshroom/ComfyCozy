...
public class WorldGenCinnamon extends WorldGenAbstractTree
{
    private IBlockState blockStateWood = ModBlocks.CINNAMON_LOG.getDefaultState().withProperty(cinnamon_log.REMAINS, Integer.valueOf(0));
  //  private IBlockState blockStateLeaves = ModBlocks.CINNAMON_LEAVES.getDefaultState();//.withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
     ...
     */
    public WorldGenCinnamon(boolean parShouldNotify)
    {
    ...
    {
        int minHeight = parRandom.nextInt(2) + minTreeHeight;
        ...
        if (parBlockPos.getY() >= 1 && parBlockPos.getY() + minHeight + 1 <= parWorld.getHeight())
        {
            ...
            else
            {
                ...
                if (state.getBlock().canSustainPlant(state, parWorld, parBlockPos.down(), EnumFacing.UP, (IPlantable) Blocks.SAPLING) && parBlockPos.getY() < parWorld.getHeight() - minHeight - 1)
                {
                    ...
                    generateLeaves(parWorld, parBlockPos, minHeight);
    ...
    private void generateLeaves(World parWorld, BlockPos parBlockPos, int height)
    {
        for (int foliageY = parBlockPos.getY() + height; foliageY <= parBlockPos.getY() + height; ++foliageY)
        {
    ...
    private void generateTrunk(World parWorld, BlockPos parBlockPos, int minHeight)
    {
        for (int height = 0; height < minHeight; ++height)
        {
            ...
            {
                setBlockAndNotifyAdequately(parWorld, parBlockPos.up(height), blockStateWood.withProperty(cinnamon_log.REMAINS, Integer.valueOf(0)));
            }
        }
    }
